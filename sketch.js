...
function drawGameInfo() {
    ...
    const boxHeight = height * 0.24; // Increased height to accommodate new info
    ...
    currentTextY += lineSpacing * 1.2; // Extra spacing before goal/day bar

    // Goal Progress Bar
    const barPaddingX = padding + 5;
    const barWidth = boxWidth - (barPaddingX * 2);
    const barHeight = height * 0.015; // Smaller bar height
    const barY = currentTextY + 5; // Position below text
    const barCornerRadius = 3;

    // Goal Text
    textSize(textBaseSize * 0.9); // Slightly smaller text for goal
    textAlign(LEFT, BOTTOM);
    text('Goal Progress:', boxX + barPaddingX, currentTextY + 3);
    ...
    // Goal Amount on bar (centered on bar)
    fill(255);
    textAlign(CENTER, CENTER);
    text(`$${gameMoney.toLocaleString()}/$${MONEY_GOAL.toLocaleString()}`, boxX + barPaddingX + barWidth / 2, barY + barHeight / 2);

    currentTextY += barHeight + lineSpacing * 1.2; // Move for next line of text

    // Day Progress Bar
    // Day Text
    textSize(textBaseSize * 0.9);
    textAlign(LEFT, BOTTOM);
    text('Day Progress:', boxX + barPaddingX, currentTextY + 3);
    ...
    fill(200, 200, 50); // Yellow for day progress
    rect(boxX + barPaddingX, currentTextY + 5, dayProgress, barHeight, barCornerRadius);

    // Day Count on bar (centered on bar)
    fill(255);
    textAlign(CENTER, CENTER);
    text(`${gameDay}/${DAY_LIMIT} Days`, boxX + barPaddingX + barWidth / 2, currentTextY + 5 + barHeight / 2);
